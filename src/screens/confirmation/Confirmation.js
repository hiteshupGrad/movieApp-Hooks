...
const Confirmation = (props) => {
  ...
  useEffect(() => {
    const price =
      ...
      parseInt(props.location.bookingSummary.tickets.length, 10);
    setTotalPrice(price);
  ...
  const confirmBookingHandler = () => {
    ...
    xhr.addEventListener("readystatechange", function () {
      if (this.readyState === 4) {
        setBookingId(JSON.parse(this.responseText).reference_number);
      }
    });

    xhr.open("POST", props.baseUrl + "bookings");
  ...
  const couponApplyHandler = () => {
    ...
    xhr.open("GET", props.baseUrl + "coupons/" + couponCode);
    xhr.setRequestHeader(
      "Authorization",
      "Bearer " + sessionStorage.getItem("access-token")
    );
